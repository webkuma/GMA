import{s as n}from"./index-fb8a7377.js";const E="/GMA";let e=null;async function o(){if(e)return e;try{const t=await n({locateFile:r=>`https://sql.js.org/dist/${r}`}),a=fetch(`${E}/music.db`).then(r=>{if(!r.ok)throw new Error("Failed to fetch database file.");return r.arrayBuffer()}),[s,i]=await Promise.all([t,a]);return e=new s.Database(new Uint8Array(i)),e}catch(t){throw console.error("Database initialization failed:",t),t}}async function N(t){return(await o()).exec(`SELECT DISTINCT * FROM shortlist WHERE id IN (${t.join(", ")}) ORDER BY year DESC`)}async function H(){return(await o()).exec("select DISTINCT year from shortlist ORDER BY year DESC")}async function l(t){return(await o()).exec(`SELECT DISTINCT awards.awards, shortlist.url
